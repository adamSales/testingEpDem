...
\begin{enumerate}
	\item Each voter is more likely to believe a candidate is
          superior when she actually is than when she is not\label{ass:belief}%Each voter is no worse---and some better---than random at judging a candidate's superiority on that issue
	\item Every voter is at least as---and some more---likely as otherwise to select a candidate if she believes  that the candidate is superior to her rival on that issue\label{ass:preference}  % 1/30 added "than otherwise"
	%\item A candidate's actual superiority affects voters' choices only through voters' perception of her superiority
...
Our claim is this: in a binary election (even when the decision function is multidimensional) where independent and minimally competent voters seek to select the candidate that maximizes a shared value, the majority rule mechanism comes to recommend democracy on veritistic grounds. %The claim is that insofar as people agree the outcome of the election can be thought to be truth-tracking by epistemic means. 
%The point here is not that the capacity to afford more Americans healthcare is sufficient for victory. Rather, to the extent that a majority of voters hold that value and are able to ascertain which candidate is better on this dimension, that candidate has his chances of victory boosted. Thus, Though the result is easiest to see in instances of value-consensus, it's not limited to such cases.  
