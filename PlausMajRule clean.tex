...
Perhaps surprisingly, Proposition \ref{prop:voteTotal} does not
necessarily imply that a candidate's superiority makes her more likely
to win the election.

One further assumption  is necessary for an increase in expected votes to reliably translate into
an increase in the probability of being elected.
...
To see why, imagine two universal preferences among the electorate, 
say higher health-care coverage rates and lower taxes.
Say candidates who are better fit to achieve higher health-care
coverage tend to be less likely to lower taxes.
Then, for the segment of voters who care more about taxes than health
coverage, a candidate's competence with regards to health-care
coverage will be \emph{anti}-correlated with her likelihood of getting
votes.
Seen from another angle, this segment of the population tends to vote
against their own preference on health-care coverage, because of their
...
\item $Pr(V_i=1|A_i=1)\ge Pr(V_i=1|A_i=0)$, stated otherwise, $A$ affects all voters' choices in the same direction.\label{ass:pref}
\item $Pr(A_i=1|a=1)\ge Pr(A_i=1|a=0)$ stated otherwise, that voters are better than random at judging $a$.\label{ass:judge}
\item For all $j\ne i$, $V_i \independent V_j|a$
\item $Pr(V_i=1|A_i, a)=Pr(V_i=1|A_i)$
\end{enumerate}
Finally, the inequalities in both \ref{ass:pref} and \ref{ass:judge}
must be strict for some non-empty subset of $\mathcal{S}$. 
