...
Let $\mathcal{E}$ denote the electorate and let
...
facts, as discussed in Section \ref{sec:interactions}.

Then consider the following assumptions:\\
For all $i\in\mathcal{E}$:
\begin{itemize}
\item[\ref{ass:preference}] $Pr(V_i=1|A_i=1,\mathcal{F})\ge Pr(V_i=1|A_i=0,\mathcal{F})$; stated otherwise, $A$ affects all voters' choices in the same direction.
\item[\ref{ass:belief}] $Pr(A_i=1|a=1,\mathcal{F})\ge Pr(A_i=1|a=0,\mathcal{F})$; stated otherwise, that voters are better than random at judging $a$.
...
\end{equation}
\end{prop*}
Note that when $\mathcal{F}$ is the empty set, this is equivalent to
Propositions \ref{prop:voteTotal} and \ref{prop:main}.
When $\mathcal{F}$ is not empty, this is equivalent to a
generalization of Proposition \ref{prop:voteTotal} and to \ref{prop:main}$'$.
